...
import { getLinearSearchSteps } from '../utils/linearSearchSteps';
...
export default function LinearSearchVisualizer() {
  ...
  useEffect(() => {
    if (targetValue !== '') {
      setSteps(getLinearSearchSteps(array, Number(targetValue)));
  ...
  const { array: currentArray, comparing, foundIndex, target } = steps[currentStepIndex];
  ...
  const handleSubmit = (e) => {
    ...
    if (parsedArray.length > 0 && targetValue !== '') {
      setArray(parsedArray);
    } else {
      alert('Please enter a valid comma-separated list of numbers and a target');
    }
  };
  ...
  const getStepDescription = () => {
    if (foundIndex !== null && foundIndex >= 0) {
      return `Target ${target} found at position ${foundIndex}`;
    } else if (comparing !== null) {
      return `Comparing target ${target} with element at position ${comparing}`;
    } else if (foundIndex === -1) {
      return `Target ${target} not found in array`;
    } else {
      return "Ready to start searching";
    }
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-blue-200 text-lg opacity-90">
            Watch how linear search scans the array to find the target
          </p>
        </div>
        ...
        <div className="mb-4">
          <div className="flex flex-col sm:flex-row gap-4 items-center justify-center">
            ...
            <input
              ...
              className="w-full sm:w-40 px-4 py-3 bg-slate-800/80 border border-blue-400/30 rounded-xl text-white placeholder-blue-300/60 focus:outline-none focus:border-blue-400/60 focus:ring-2 focus:ring-blue-400/20"
            />
            <button 
        ...
        <div className="mb-8 mt-8">
            ...
            {currentArray.map((value, index) => (
              <div
                ...
                className={`
                  ...
                  ${(comparing === index || foundIndex === index) ? 'scale-110' : 'scale-100'}
                  shadow-lg
                `}
              >
                {value}
          ...
          <div className="flex flex-wrap justify-center gap-6 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="text-blue-200">Unvisited</span>
            </div>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-green-200">Found</span>
            </div>
          </div>
        </div>
